...
class LineTShiftDownView(ctx : Context) : View(ctx) {
    ...
    data class LineTShiftDown(var i : Int) {

        private var curr : LTSDNode = LTSDNode(0)
    ...
    data class Renderer(var view : LineTShiftDownView) {

        private val ltsd : LineTShiftDown = LineTShiftDown(0)
        private val animator : Animator = Animator(view)
        private val paint : Paint = Paint(Paint.ANTI_ALIAS_FLAG)

        fun render(canvas : Canvas) {
            canvas.drawColor(backColor)
            ltsd.draw(canvas, paint)
            animator.animate {
                ltsd.update {
                    animator.stop()
                }
