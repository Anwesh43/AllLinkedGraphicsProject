...
class SquaresRotPushLineView(ctx : Context) : View(ctx) {
    ...
    data class SRPLNode(var i : Int, val state : State = State()) {

        private var next : SRPLNode? = null
        private var prev : SRPLNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawSRPLNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : SRPLNode {
            var curr : SRPLNode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class SquaresRotPushLine(var i : Int) {

        private var curr : SRPLNode = SRPLNode(0)
    ...
    data class Renderer(var view : SquaresRotPushLineView) {

        private val animator : Animator = Animator(view)
        private val srpl : SquaresRotPushLine = SquaresRotPushLine(0)
        private val paint : Paint = Paint(Paint.ANTI_ALIAS_FLAG)

        fun render(canvas : Canvas) {
            canvas.drawColor(backColor)
            srpl.draw(canvas, paint)
            animator.animate {
                srpl.update {
                    animator.stop()
                }
