package com.example.downrotarrowview
...
class DownRotArrowView(ctx : Context) : View(ctx) {
    ...
    data class DRANode(var i : Int, val state : State = State()) {

        private var prev : DRANode? = null
        private var next : DRANode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawDRANode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : DRANode {
            var curr : DRANode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class DownRotArrow(var i : Int) {

        private var curr : DRANode = DRANode(0)
