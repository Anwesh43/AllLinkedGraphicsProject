package com.example.twolinetosquaredownview
...
fun Canvas.drawTwoLineSquareDown(scale : Float, w : Float, h : Float, paint : Paint) {
...
fun Canvas.drawTLSDNode(i : Int, scale : Float, paint : Paint) {
    val w : Float = width.toFloat()
    val h : Float = height.toFloat()
    paint.color = colors[i]
    paint.strokeCap = Paint.Cap.ROUND
    paint.strokeWidth = Math.min(w, h) / strokeFactor
    drawTwoLineSquareDown(scale, w, h, paint)
}

class TwoLineSquareDownView(ctx : Context) : View(ctx) {
    ...
    data class TwoLineSquareDown(var i : Int) {

        private var curr : TLSDNode = TLSDNode(0)
