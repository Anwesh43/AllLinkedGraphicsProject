...
class ArcJoinLineDownView(ctx : Context) : View(ctx) {
    ...
    data class AJLDNode(var i : Int, val state : State = State()) {

        private var prev : AJLDNode? = null
        private var next : AJLDNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawAJLDNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : AJLDNode {
            var curr : AJLDNode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class ArcJoinLineDown(var i : Int) {

        private var curr : AJLDNode = AJLDNode(0)
    ...
    data class Renderer(var view : ArcJoinLineDownView) {
        ...
        private val ajld : ArcJoinLineDown = ArcJoinLineDown(0)
