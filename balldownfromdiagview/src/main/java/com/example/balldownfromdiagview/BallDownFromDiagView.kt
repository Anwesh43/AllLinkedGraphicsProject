...
class BallDownFromDiagView(ctx : Context) : View(ctx) {
    ...
    data class BDFDNode(var i : Int, val state : State = State()) {

        private var next : BDFDNode? = null
        private var prev : BDFDNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawBDFDNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : BDFDNode {
            var curr : BDFDNode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class BallDownFromDiag(var i : Int) {

        private var curr : BDFDNode = BDFDNode(0)
    ...
    data class Renderer(var view : BallDownFromDiagView) {

        private val bdd : BallDownFromDiag = BallDownFromDiag(0)
