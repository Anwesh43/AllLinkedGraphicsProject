...
class ParallelLineTravellerView(ctx : Context) : View(ctx) {
    ...
    data class PLTNode(var i : Int, val state : State = State()) {

        private var next : PLTNode? = null
        private var prev : PLTNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawPLTNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : PLTNode {
            var curr : PLTNode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class ParallelLineTraveller(var i : Int) {

        private var curr : PLTNode = PLTNode(0)
