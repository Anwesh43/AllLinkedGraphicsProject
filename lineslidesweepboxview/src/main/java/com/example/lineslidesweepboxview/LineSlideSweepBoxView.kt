package com.example.lineslidesweepboxview
...
fun Canvas.drawLineSlideSweepBox(scale : Float, w : Float, h : Float, paint : Paint) {
...
fun Canvas.drawLSSBNode(i : Int, scale : Float, paint : Paint) {
    val w : Float = width.toFloat()
    val h : Float = height.toFloat()
    paint.color = colors[i]
    paint.strokeCap = Paint.Cap.ROUND
    paint.strokeWidth = Math.min(w, h) / strokeFactor
    drawLineSlideSweepBox(scale, w, h, paint)
}

class LineSlideSweepBoxView(ctx : Context) : View(ctx) {
    ...
    data class LineSlideSweepBox(var i : Int) {

        private var curr : LSSBNode = LSSBNode(0)
