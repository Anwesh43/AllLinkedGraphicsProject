...
class LineBarTravellerView(ctx : Context) : View(ctx) {
    ...
    data class LBTNode(var i : Int, val state : State = State()) {

        private var next : LBTNode? = null
        private var prev : LBTNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawLBTNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : LBTNode {
            var curr : LBTNode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class LineBarTraveller(var i : Int) {

        private var curr : LBTNode = LBTNode(0)
