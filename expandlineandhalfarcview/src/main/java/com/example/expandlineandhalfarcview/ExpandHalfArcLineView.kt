...
class ExpandHalfArcLineView(ctx : Context) : View(ctx) {
    ...
    data class EHALNode(var i : Int, val state : State = State()) {

        private var prev : EHALNode? = null
        private var next : EHALNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawEHALNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : EHALNode {
            var curr : EHALNode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class ExpandHalfArcLine(var i : Int) {

        private var curr : EHALNode = EHALNode(0)
    ...
    data class Renderer(var view : ExpandHalfArcLineView) {

        private val animator : Animator = Animator(view)
        private val ehal : ExpandHalfArcLine = ExpandHalfArcLine(0)
        private val paint : Paint = Paint(Paint.ANTI_ALIAS_FLAG)

        fun render(canvas : Canvas) {
            canvas.drawColor(backColor)
            ehal.draw(canvas, paint)
            animator.animate {
                ehal.update {
                    animator.stop()
                }
