package com.example.squareballcatcherview
...
fun Canvas.drawSBCNode(i : Int, scale : Float, paint : Paint) {
    val w : Float = width.toFloat()
    val h : Float = height.toFloat()
    paint.color = colors[i]
    paint.strokeCap = Paint.Cap.ROUND
    paint.strokeWidth = Math.min(w, h) / strokeFactor
    drawSquareBallCatcher(scale, w, h, paint)
}

class SquareBallCatcherView(ctx : Context) : View(ctx) {
    ...
    data class SBCNode(var i : Int, val state : State = State()) {

        private var next : SBCNode? = null
        private var prev : SBCNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawSBCNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : SBCNode {
            var curr : SBCNode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class SquareBallCatcher(var i : Int) {

        private var curr : SBCNode = SBCNode(i + 1)
