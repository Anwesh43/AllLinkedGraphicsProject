...
class PerpAngleSquareView(ctx : Context) : View(ctx) {
    ...
    data class PASNode(var i : Int, val state : State = State()) {
        private var prev : PASNode? = null
        private var next : PASNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawPASNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : PASNode {
            var curr : PASNode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class PerpAngleSquare(var i : Int) {

        private var curr : PASNode = PASNode(0)
