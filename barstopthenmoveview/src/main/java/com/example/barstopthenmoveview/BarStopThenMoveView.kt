package com.example.barstopthenmoveview
...
fun Canvas.drawBarStopThenMove(scale : Float, w : Float, h : Float, paint : Paint) {
    ...
    translate(w / 2, h / 2)
    save()
    translate(0f, -h / 2 + h * 0.5f * sc2 + (h * 0.5f + size) * sc4)
    drawRect(RectF(-size / 4, -size, size / 4, 0f), paint)
    restore()
    drawLine(-w / 2 + w * sc3, 0f, -w / 2 + w * sc1, 0f, paint)
    restore()
}

fun Canvas.drawBSTMNode(i : Int, scale : Float, paint : Paint) {
...
class BarStopThenMoveView(ctx : Context) : View(ctx) {
    ...
    data class BarStopThenMove(var i : Int, val state : State = State()) {

        private var curr : BSTMNode = BSTMNode(0)
