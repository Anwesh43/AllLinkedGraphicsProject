package com.example.thicklinerotview
...
fun Canvas.drawThickRotLine(scale : Float, w : Float, h : Float, paint : Paint) {
    ...
    drawRect(RectF(-size * 0.125f, -size * sc2, size * 0.125f, 0f), paint)
    restore()
}
...
fun Canvas.drawTLRNode(i : Int, scale : Float, paint : Paint) {
    ...
    drawThickRotLine(scale, w, h, paint)
}

class ThickLineRotView(ctx : Context) :  View(ctx) {
    ...
    data class ThickLineRot(var i : Int) {
    ...
    data class Animator(var view : ThickLineRotView, var animated : Boolean = false) {
