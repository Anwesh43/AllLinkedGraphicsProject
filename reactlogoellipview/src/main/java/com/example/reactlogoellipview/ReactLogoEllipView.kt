...
class ReactLogoEllipView(ctx : Context) : View(ctx) {
    ...
    data class RLENode(var i : Int, val state : State = State()) {

        private var next : RLENode? = null
        private var prev : RLENode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawRLENode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : RLENode {
            var curr : RLENode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class ReactLogoEllip(var i : Int) {

        private var curr : RLENode = RLENode(0)
    ...
    data class Renderer(var view : ReactLogoEllipView) {

        private val animator : Animator = Animator(view)
        private val rle : ReactLogoEllip = ReactLogoEllip(0)
        private val paint : Paint = Paint(Paint.ANTI_ALIAS_FLAG)

        fun render(canvas : Canvas) {
            canvas.drawColor(backColor)
            rle.draw(canvas, paint)
            animator.animate {
                rle.update {
                    animator.stop()
                }
