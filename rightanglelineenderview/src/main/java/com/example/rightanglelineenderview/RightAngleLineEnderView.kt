...
class RightAngleLineEnderView(ctx : Context) : View(ctx) {
    ...
    data class RALENode(var i : Int, val state : State = State()) {

        private var prev : RALENode? = null
        private var next : RALENode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawRALENode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : RALENode {
            var curr : RALENode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class RightAngleLineEnder(var i : Int) {

        private var curr : RALENode = RALENode(0)
