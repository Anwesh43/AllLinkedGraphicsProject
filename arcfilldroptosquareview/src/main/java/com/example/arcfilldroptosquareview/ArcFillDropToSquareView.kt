...
class ArcFillDropToSquareView(ctx : Context) : View(ctx) {
    ...
    data class AFDTSNode(var i : Int, val state : State = State()) {

        private var prev : AFDTSNode? = null
        private var next : AFDTSNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawAFDTSNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : AFDTSNode {
            var curr : AFDTSNode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class ArcFillDropToSquare(var i : Int) {

        private var curr : AFDTSNode = AFDTSNode(0)
