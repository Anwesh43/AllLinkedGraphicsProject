...
class CrossToUpArrowView(ctx : Context) : View(ctx) {
    ...
    data class CTUANode(var i : Int, val state : State = State()) {

        private var next : CTUANode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawCTUANode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : CTUANode {
            var curr : CTUANode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
