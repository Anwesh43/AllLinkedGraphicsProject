...
fun Canvas.drawTrapPath(scale : Float, w : Float, h : Float, paint : Paint) {
...
fun Canvas.drawTPMNode(i : Int, scale : Float, paint : Paint) {
    val w : Float = width.toFloat()
    val h : Float = height.toFloat()
    paint.color = colors[i]
    paint.strokeCap = Paint.Cap.ROUND
    paint.strokeWidth = Math.min(w, h) / strokeFactor
    drawTrapPath(scale, w, h, paint)
}

class TrapPathMoveView(ctx : Context) : View(ctx) {
    ...
    data class TPMNode(var i : Int, val state : State = State()) {

        private var next : TPMNode? = null
        private var prev : TPMNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawTPMNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : TPMNode {
            var curr : TPMNode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class TrapPathMove(var i : Int) {

        private var curr : TPMNode = TPMNode(0)
