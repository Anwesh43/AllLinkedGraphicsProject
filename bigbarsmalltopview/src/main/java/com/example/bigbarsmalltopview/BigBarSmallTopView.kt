...
class BigBarSmallTopView(ctx : Context) : View(ctx) {
    ...
    data class BigBarSmallTop(var i : Int) {

        private var curr : BBSTNode = BBSTNode(0)
    ...
    data class Renderer(var view : BigBarSmallTopView) {

        private val animator : Animator = Animator(view)
        private val bbst : BigBarSmallTop = BigBarSmallTop(0)
        private val paint : Paint = Paint(Paint.ANTI_ALIAS_FLAG)

        fun render(canvas : Canvas) {
            canvas.drawColor(backColor)
            bbst.draw(canvas, paint)
            animator.animate {
                bbst.update {
                    animator.stop()
                }
