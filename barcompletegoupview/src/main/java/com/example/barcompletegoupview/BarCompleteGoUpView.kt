...
class BarCompleteGoUpView(ctx : Context) : View(ctx) {
    ...
    data class BCGUNode(var i : Int, val state : State = State()) {

        private var next : BCGUNode? = null
        private var prev : BCGUNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawBCGUNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : BCGUNode {
            var curr : BCGUNode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class BarCompleteGoUp(var i : Int) {

        private var curr : BCGUNode = BCGUNode(0)
