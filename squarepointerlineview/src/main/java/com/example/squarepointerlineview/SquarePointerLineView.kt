package com.example.squarepointerlineview
...
fun Canvas.drawSquarePointerLine(scale : Float, w : Float, h : Float, paint : Paint) {
    ...
    restore()
    drawRect(RectF(-sqSize * sc2, -sqSize * sc2, sqSize * sc2, sqSize * sc2), paint)
    restore()
}

fun Canvas.drawSPLNode(i : Int, scale : Float, paint : Paint) {
    val w : Float = width.toFloat()
    val h : Float = height.toFloat()
    paint.color = colors[i]
    paint.strokeCap = Paint.Cap.ROUND
    paint.strokeWidth = Math.min(w, h) / strokeFactor
    drawSquarePointerLine(scale, w, h, paint)
}

class SquarePointerLineView(ctx : Context) : View(ctx) {
    ...
    data class SquarePointerLine(var i : Int) {

        private var curr : SPLNode = SPLNode(0)
