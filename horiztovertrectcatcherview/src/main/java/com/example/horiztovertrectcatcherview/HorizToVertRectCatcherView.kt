package com.example.horiztovertrectcatcherview
...
fun Canvas.drawHorizToVertRectCatcher(scale : Float, w : Float, h : Float, paint : Paint) {
...
fun Canvas.drawHTVRCNode(i : Int, scale : Float, paint : Paint) {
...
class HorizToVertRectCatcherView(ctx : Context) : View(ctx) {
    ...
    data class HorizToVertRectCatcher(var i : Int) {

        private var curr : HTVRCNode = HTVRCNode(0)
    ...
    data class Renderer(var view : HorizToVertRectCatcherView) {

        private val animator : Animator = Animator(view)
        private val horizToVertRectCatcher : HorizToVertRectCatcher = HorizToVertRectCatcher(0)
        private val paint : Paint = Paint(Paint.ANTI_ALIAS_FLAG)

        fun render(canvas : Canvas) {
            canvas.drawColor(backColor)
            horizToVertRectCatcher.draw(canvas, paint)
            animator.animate {
                horizToVertRectCatcher.update {
                    animator.stop()
                }
            }
        }
        ...
        fun handleTap() {
            horizToVertRectCatcher.startUpdating {
                animator.start()
            }
        }
    }
