package com.example.linefromendvsweepview
...
fun Canvas.drawLineFromEndVSweep(scale : Float, w : Float, h : Float, paint : Paint) {
...
fun Canvas.drawLFEVSNode(i : Int, scale : Float, paint : Paint) {
    val w : Float = width.toFloat()
    val h : Float = height.toFloat()
    paint.color = colors[i]
    paint.strokeCap = Paint.Cap.ROUND
    paint.strokeWidth = Math.min(w, h) / strokeFactor
    drawLineFromEndVSweep(scale, w, h, paint)
}

class LineFromEndVSweepView(ctx : Context) : View(ctx) {
    ...
    data class LineFromEndVSweep(var i : Int) {

        private var curr : LFEVSNode = LFEVSNode(0)
