...
class LLineArcView(ctx : Context) : View(ctx) {
    ...
    data class LLANode(var i : Int, val state : State = State()) {

        private var next : LLANode? = null
        private var prev : LLANode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawLLANode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : LLANode {
            var curr : LLANode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class LLineArc(var i : Int) {

        private var curr : LLANode = LLANode(0)
